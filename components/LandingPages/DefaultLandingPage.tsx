...
// export enum backgroundSpreadingVars{
//     ltr = "",
//     rtl = ""
// }
                ...
                if(bgCrossFilling)
                    if(i % 2 === 0) {
                        newArr.push({
                            size: blockSize,
                            content: {
                                data: elemsContent.length >= contentIndex ? elemsContent[contentIndex].content.data : null,
                                contentDistance: elemsContent.length >= contentIndex ? elemsContent[contentIndex].content.contentDistance : null,
                                contentOffset: elemsContent.length >= contentIndex ? elemsContent[contentIndex].content.contentOffset : null,
                            }
                        })
                    }
                    else {
    ...
    const parallax = useCallback((e) => {
        ...
        if(bgElems){
            bgElems.forEach(elem => {
                    ...
                    const x = (window.innerWidth - e.pageX * (5 - (1-parseFloat(speed)) * 10))/100;
                    const y = (window.innerWidth - e.pageY * (5 - (1-parseFloat(speed)) * 10))/100;
        ...
        <>
            <div className={styles.defaultLandingPage} id={pageId} ref={defPage}>
                <div style={{
                    ...
                    maxWidth: `${rowsBlockSize ? rowsBlockSize.toString() + "px" : "auto"}`,

                    }}>

                    {elemsInMatrix ? elemsInMatrix.map((elem, index) => (
                            <div style={{
                                width: elem.size * backMatrix.colsNum <= width ? elem.size
                                ...
                                {elem.content ? (
                                    <div
                                        style={{
                                            ...
                                            filter: `blur(${10 - elem.content.contentDistance * 10}px)`,
                ...
                <div className={"container-fluid fluid-adaptive d-flex flex-column"}>
                    <div className={"row"}>
                        <div className={"col-md-7 d-flex h-100 flex-column"}>
                            <MainLabel
                                padding={landingDescription?.titleOffset ? landingDescription?.titleOffset : 0}
